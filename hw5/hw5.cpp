...
ColumnVector Find_Basic_Solution(Matrix A, ColumnVector y)
{
    ...
    //if rank of A equals to the minimun of A's col or row #
    if (Rank(A) == min(A.ncols(), A.nrows()))
    {
        //return z in equation 1
        ...
        do
        {
            ...
            for (int i = 0; i < A.ncols(); ++i)
            {
                ...
                if (did_i_pick_this_col[i]){
                    //push back an empty temp vector
                    B.push_back(temp);
                    //loop for rows
                    for (int j = 1; j <= A.nrows(); j++){
                        //push back rows to fill
...
{
    Matrix solution(A.ncols(), (double)solution_set.size());
